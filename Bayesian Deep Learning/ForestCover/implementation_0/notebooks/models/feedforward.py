...
class ForestCoverModel(BayesianModel):
    ...
    def create_model(self, X=None, y=None):
        ...
        with pm.Model() as model:
            # Define weights
            weights_1 = pm.Normal('w_1', mu=0, sd=1, shape=(self.num_pred, self.n_hidden))
            weights_2 = pm.Normal('w_2', mu=0, sd=1, shape=(self.n_hidden, self.n_hidden))
            weights_out = pm.Normal('w_out', mu=0, sd=1, shape=(self.n_hidden, self.num_out))
            ...
            # Define likelihood
            out = pm.Multinomial('likelihood', n=1, p=acts_out, observed=model_output)
    ...
    def fit(self, X, y, n=200000, batch_size=10):
        ...
        if self.cached_model is None:
            self.cached_model = self.create_model(X, y)

        with self.cached_model:

            y_minibatch = pm.Minibatch(y, batch_size=batch_size)
            
            minibatches = {
                self.shared_vars['model_input']     :  pm.Minibatch(X, batch_size=batch_size),
                self.shared_vars['model_output']    :  y_minibatch
            }
